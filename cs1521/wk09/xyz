...
int main(int argc, char *argv[], char *envp[])
{
   ...
   while (fgets(line, BUFSIZ, stdin) != NULL) {
      ...
      char **command_line = tokenise(line, " ");
      if ((pid = fork()) != 0) {
	      wait(&stat);
      } else{
	      execute(command_line, path, envp);
      }
...
void execute(char **args, char **path, char **envp)
{
   ...
   if (args[0][0] == '/' || args[0][0] == '.') {
      ...
      int i = 0;
      while (path[i] != NULL) {
         char *search = malloc(strlen(args[0]) + strlen(path[i]) + 1);
	      strcpy(search, path[i]);
	      strcat(search, "/");
	      strcat(search, args[0]);
	      if (isExecutable(search)){
	         command = search;
	         break;
	      }
	      i++;
   ...
   if (command == NULL) {
      ...
      printf("Executing: %s\n", command);
      int exe = execve(command, args, envp);
      if (exe == -1) {
         perror("Exec failed");
      }
   }
